...
4. Photos (raw/figure entries only): chat can't transmit image bytes, so Max delivers them separately — one paste, not a file hunt. Give him the `drop_page` URL from `pearl_status` (never hardcode it; it carries a secret key and this repo is public) with one line: **"paste the screenshot there (⌘V) — or tap it on your phone to snap the photo"**. Ask BEFORE writing the entry so he can paste while you work. Then just reference `entries/img/{id}-1.jpg` (`-2.jpg`, … in display order) in the fragment and call `stage_pearl` normally — **wiring is automatic**: any referenced image not already in the repo is filled from what he pasted, in order, and cleared from the inbox. If not enough photos are waiting, staging FAILS with a message saying how many more he owes you — nothing is staged, so a preview can never show a broken image. Check `images_used` in the result and say which photo landed where if there was more than one.
